...
trait StaminoTestKit { self: org.scalatest.wordspec.AnyWordSpecLike =>

  val defaultSampleId = "default"
  case class PersistableSample[FromVersion <: Version: VersionInfo](
    sampleId: String,
    persistable: AnyRef,
    description: Option[String],
  ) {
    override def toString =
      SimpleClassNameExtractor(persistable.getClass) + description.map(" " + _).getOrElse("")
    val fromVersionNumber = implicitly[VersionInfo[FromVersion]].versionNumber

    def from[NewFromVersion <: Version: VersionInfo] =
      PersistableSample[NewFromVersion](sampleId, persistable, description)
  }

  def sample(persistable: AnyRef) = new PersistableSample[V1](defaultSampleId, persistable, None)
  def sample(sampleId: String, persistable: AnyRef) =
    new PersistableSample[V1](sampleId, persistable, Some(sampleId))
  def sample(sampleId: String, persistable: AnyRef, description: String) =
    new PersistableSample[V1](sampleId, persistable, Some(description))

  implicit class TestablePersisters(persisters: Persisters)
    ...
    private def generateRoundtripTestFor(sample: PersistableSample[_]) = {
      s"persist and unpersist $sample" in {
        persisters.unpersist(persisters.persist(sample.persistable)) should equal(
          sample.persistable,
        )
      }
    }

    private def generateStoredVersionsDeserializationTestsFor(
      sample: PersistableSample[_],
    ): Unit = {
      latestVersion(sample.persistable).map(latestVersion =>
        Range
          .inclusive(sample.fromVersionNumber, latestVersion)
          .foreach { version =>
    ...
    def latestVersion(persistable: AnyRef) = Try(
      persisters.persisters.filter(_.canPersist(persistable)).map(_.currentVersion).max,
    ).toOption

    private def verifyByteStringDeserialization(
      sample: PersistableSample[_],
      version: Int,
      latestVersion: Int,
    ): Unit = {
      ...
      byteStringFromResource(serialized.key, version, sample.sampleId) match {
        ...
        case Failure(_: java.io.FileNotFoundException) if version == latestVersion =>
          val writtenToPath = saveByteArrayToTargetSerializationDirectory(
            serialized.bytes.toArray,
            serialized.key,
            version,
            sample.sampleId,
          )
          fail(
            s"You appear to have added a new serialization sample to the stamino persisters' test.\n" +
        ...
        case Failure(_: java.io.FileNotFoundException) if version < latestVersion =>
          fail(
            s"While testing that the older serialized version $version of sample with key ${serialized.key} and sample id ${sample.sampleId} was not found",
          )
        case Failure(other) =>
          fail(
            s"Failure while decoding serialized version $version of sample with key ${serialized.key} and sample id ${sample.sampleId} was not found",
            other,
          )
      }
    }

    private def decodeBase64(s: String): Try[Array[Byte]] = Try(
    ...
    private def byteStringFromResource(
      key: String,
      version: Int,
      sampleId: String,
    ): Try[Persisted] = {
    ...
    private def saveByteArrayToTargetSerializationDirectory(
      bytes: Array[Byte],
      key: String,
      version: Int,
      sampleId: String,
      ...
      val path =
        Paths.get(targetDirectoryForExampleSerializations, filename(key, version, sampleId))
      Files.write(
        path,
        encodeBase64(bytes),
    ...
    private def filename(key: String, version: Int, sampleId: String) =
      s"$key-v$version-" + sampleId.replaceAll("\\s+", "-")
