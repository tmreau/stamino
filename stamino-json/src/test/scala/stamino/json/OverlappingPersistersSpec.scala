...
class OverlappingPersisterSpec extends StaminoJsonSpec {
  ...
  "The persisters construction DSL" should {
    ...
    "correctly handle overlapping persisters" in {
      ...
      e.getMessage() should be(
        "requirement failed: Overlapping persisters: Persisters with keys 'payload1', 'payload2' all persist class stamino.json.OverlappingPersisterSpecDomain$Event.",
      )
