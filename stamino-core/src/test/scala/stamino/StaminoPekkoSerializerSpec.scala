...
class StaminoPekkoSerializerSpec extends StaminoSpec {
  import TestDomain._
  ...
  class MyPekkoSerializer1a
      extends StaminoPekkoSerializer(List(itemPersister, cartPersister, cartCreatedPersister))
  class MyPekkoSerializer1b
      extends StaminoPekkoSerializer(
        List(itemPersister, cartPersister, cartCreatedPersister),
        DefaultPersistedCodec,
      )
  class MyPekkoSerializer2
      extends StaminoPekkoSerializer(itemPersister, cartPersister, cartCreatedPersister)

  val serializer = new MyPekkoSerializer1a

  import serializer._

  "The StaminoPekkoSerializer" should {
