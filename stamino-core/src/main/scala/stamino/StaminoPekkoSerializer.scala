...
/** A custom Pekko Serializer specifically designed for use with Pekko Persistence.
  */
abstract class StaminoPekkoSerializer private[stamino] (
  persisters: Persisters,
  codec: PersistedCodec,
) extends Serializer {
  def this(persisters: List[Persister[_, _]], codec: PersistedCodec = DefaultPersistedCodec) =
    this(Persisters(persisters), codec)
  def this(persister: Persister[_, _], persisters: Persister[_, _]*) =
    this(Persisters(persister :: persisters.toList), DefaultPersistedCodec)
  ...
  /** @throws UnregisteredTypeException
    *   when the specified object is not supported by the persisters.
    */
  ...
  /** @throws UnsupportedDataException
    *   when the persisted key and/or version is not supported.
    * @throws UnrecoverableDataException
    *   when the key and version are supported but recovery throws an exception.
    */
